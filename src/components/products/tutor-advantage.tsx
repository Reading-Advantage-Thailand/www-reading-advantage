...
export default function TutorAdvantage() {
  ...
  return (
    <section className="bg-gradient-to-b from-green-200 to-green-400 text-green-900 py-16">
      <div className="container px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
        <div className="max-w-4xl mx-auto text-center bg-green-300/50 rounded-2xl p-8 backdrop-blur-sm">
          <motion.div
          ...
          >
            {features.map((feature) => (
              <motion.div
                key={feature.title}
                variants={item}
                className="bg-green-400/30 p-6 rounded-lg backdrop-blur-sm hover:bg-green-500/40 transition-colors"
              >
                <feature.icon className="w-8 h-8 mx-auto mb-3" strokeWidth={1.5} />
                <h3 className="font-semibold mb-2">{feature.title}</h3>
                <p className="text-green-800/90">{feature.description}</p>
              </motion.div>
            ))}
          </motion.div>
